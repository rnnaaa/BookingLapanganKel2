...
try {
  if ($aksi === 'valid' || $aksi === 'tolak') {
    ...
    if ($aksi === 'valid') {
      ...
      // KEUANGAN: hanya jika tipe = Pelunasan -> masukkan 1x jumlah keseluruhan (total_amount), cek anti-duplikat
    ...
    // Ambil booking dan cek remaining
    $sql_b = "SELECT b.*, u.nama AS nama_user, l.nama_lapangan FROM booking b JOIN users u ON b.id_user=u.id_user JOIN lapangan l ON b.id_lapangan=l.id_lapangan WHERE b.id_booking = ? LIMIT 1";
    $stmt_b = mysqli_prepare($conn, $sql_b);
    mysqli_stmt_bind_param($stmt_b, "i", $booking_id);
    mysqli_stmt_execute($stmt_b);
    ...
    mysqli_commit($conn);
    $_SESSION['success'] = "✅ Pelunasan berhasil diproses. Booking #{$booking_id} LUNAS dan sudah dicatat ke tabel keuangan.";
  } else {
