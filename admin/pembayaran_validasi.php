...
if(!$id || !in_array($aksi,['valid','tolak'])){
...
try{
  if($aksi==='valid'){
    mysqli_query($conn,"
      UPDATE pembayaran 
      SET status_verifikasi='valid', verified_at=NOW()
      WHERE id_pembayaran='$id'
    ");
    if($p['tipe']==='DP'){
      mysqli_query($conn,"
        UPDATE booking 
        SET payment_status='dp_bayar', status='disetujui', updated_at=NOW()
        WHERE id_booking='{$p['id_booking']}'
      ");
    }else{
      mysqli_query($conn,"
        UPDATE booking 
        SET payment_status='lunas', status='selesai', updated_at=NOW()
        WHERE id_booking='{$p['id_booking']}'
      ");
    }
  }else{
    mysqli_query($conn,"
      UPDATE pembayaran 
      SET status_verifikasi='tidak_valid', verified_at=NOW()
      WHERE id_pembayaran='$id'
    ");
    mysqli_query($conn,"
      UPDATE booking 
      SET payment_status='menunggu_verifikasi', status='menunggu', updated_at=NOW()
      WHERE id_booking='{$p['id_booking']}'
    ");
  }
  mysqli_commit($conn);
