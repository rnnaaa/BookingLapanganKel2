<?php
// booking_detail.php
// FIX: Menambahkan kolom info_produk ke rincian keuangan & Perbaikan UI
...
// 2. Ambil data booking + user + lapangan + admin approval
$sql = "
    SELECT 
      b.*, 
      u.nama AS nama_user, 
...
// 3. Ambil jadwal main & Hitung Total Harga Lapangan Murni
$stmt = $conn->prepare("
    SELECT 
        jw.jam_mulai, 
        jw.jam_selesai,
...
// Hitung total harga murni lapangan (untuk memisahkan dengan biaya tambahan)
$total_harga_lapangan = 0;
$jadwal_items = []; // Simpan data jadwal ke array agar bisa di-looping 2 kali
...
// Total di database dikurangi total harga slot lapangan
$biaya_tambahan = floatval($data['total_amount']) - $total_harga_lapangan;

// 4. Ambil data pembayaran (DP & pelunasan)
$stmt = $conn->prepare("
    SELECT 
        p.*,
                ...
                <div class="card-body">
                            ...
                            <?php if (!empty($data['info_produk'])): ?>
                                <div class="card border-info">
                                    <div class="card-header bg-info text-white">
                                        <h6 class="mb-0"><i class="fas fa-boxes me-2"></i> Tambahan Produk/Minuman</h6>
                                    </div>
                                    <div class="card-body bg-light">
                                        <p class="mb-0 small fst-italic"><?= nl2br(htmlspecialchars($data['info_produk'])) ?></p>
                                    </div>
                                </div>
                            <?php endif; ?>
                        </div>
                    ...
                    <div class="mb-4 mt-4">
                        ...
                        <div class="d-flex flex-wrap gap-2">
                            <?php foreach ($jadwal_items as $j): ?>
                                <span class="badge bg-light text-dark border p-2 fs-6" title="Harga: Rp <?= number_format($j['harga'], 0, ',', '.') ?>">
                    ...
                    <div class="card border-warning mb-3">
                        ...
                        <div class="card-body">
                            ...
                            <h6 class="mb-3"><i class="fas fa-list-alt"></i> Detail Tagihan</h6>
                            <div class="table-responsive mb-4">
                                <table class="table table-sm table-striped">
                                    <tbody>
                                        <tr>
                                            <th width="40%">Total Harga Lapangan</th>
                                            <td class="text-end">Rp <?= number_format($total_harga_lapangan, 0, ',', '.') ?></td>
                                        </tr>
                                        ...
                                        <tr>
                                            <th>
                                                ...
                                                <?php if (!empty($data['info_produk'])): ?>
                                                    <br><small class="text-muted fw-normal fst-italic">Item: <?= htmlspecialchars(substr($data['info_produk'], 0, 50)) ?><?= strlen($data['info_produk']) > 50 ? '...' : '' ?></small>
                                                <?php endif; ?>
                                            </th>
                                            <td class="text-end">Rp <?= number_format($biaya_tambahan, 0, ',', '.') ?></td>
                                        </tr>
                                        <?php endif; ?>

                                        <tr class="table-primary fw-bold">
                                            <th>GRAND TOTAL TAGIHAN</th>
                                            <td class="text-end">Rp <?= number_format($data['total_amount'], 0, ',', '.') ?></td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
